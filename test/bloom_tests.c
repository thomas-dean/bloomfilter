...
static int simple_test(void)
{
    ...
    assert(bloom_add(&bloom, "Hello world!", 12) == -1);   /* Should fail */
    assert(bloom_check(&bloom, "Hello world!", 12) == -1); /* Should fail */
    assert(bloom_init(&bloom, 1.00) == 1);                 /* Should fail */
    assert(bloom_init(&bloom, 0.00) == 1);                 /* Should fail */
    assert(bloom_init(&bloom, 0.50) == 0);                 /* Can reinitialize */
    bloom_free(&bloom);
